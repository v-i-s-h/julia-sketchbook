...
xt1s = Array([[x1s[i] + 0.50; x2s[i] + 0.50] for i ∈ 1:M])
x1s = rand(M) * 4.5; x2s = rand(M) * 4.5;
append!(xt1s, Array([[x1s[i] - 5.0; x2s[i] - 5.0] for i ∈ 1:M]))

x1s = rand(M) * 4.5; x2s = rand(M) * 4.5;
xt0s = Array([[x1s[i] + 0.5; x2s[i] - 5] for i ∈ 1:M])
x1s = rand(M) * 4.5; x2s = rand(M) * 4.5;
append!(xt0s, Array([[x1s[i] - 5.0; x2s[i] + 0.5] for i ∈ 1:M]))
...
# Fund the index that provided the highest log posterior in the chain
_, i = findmax(ch[:lp])

# Exatract the max row value from i
i = i.I[1]
