...
function diffeqsol_to_array(
    x::ODESolution{T,N,<:AbstractVector{<:CuArray}}
) where {T,N}
...
function train()
    ...
    for epoch in 1:nepochs
        stime = time()
        iterator =
            CUDA.functional() ? CuIterator(train_dataloader) : train_dataloader
        for (x, y) in iterator
